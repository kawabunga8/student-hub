...
do $$ begin
  ...
  if not exists (select 1 from public.classes where name = 'Computer Studies 10' and block_label = 'G' and school_year = '2026-27') then
    insert into public.classes (name, block_label, room, school_year, sort_order) values ('Computer Studies 10', 'G', '124', '2026-27', 7);
  end if;
  ...
  if not exists (select 1 from public.classes where name = 'ICT 9 Q1' and block_label = 'H' and school_year = '2026-27') then
    insert into public.classes (name, block_label, room, school_year, sort_order, active_quarters) values ('ICT 9 Q1', 'H', '124', '2026-27', 8, array[1]);
  end if;

  if not exists (select 1 from public.classes where name = 'ICT 9 Q2' and block_label = 'H' and school_year = '2026-27') then
    insert into public.classes (name, block_label, room, school_year, sort_order, active_quarters) values ('ICT 9 Q2', 'H', '124', '2026-27', 9, array[2]);
  end if;

  if not exists (select 1 from public.classes where name = 'Band 9' and block_label = 'H' and school_year = '2026-27') then
    insert into public.classes (name, block_label, room, school_year, sort_order, active_quarters) values ('Band 9', 'H', '130', '2026-27', 10, array[3,4]);
  end if;

  if not exists (select 1 from public.classes where name = 'Career Class 10' and block_label = 'CL' and school_year = '2026-27') then
    insert into public.classes (name, block_label, room, school_year, sort_order) values ('Career Class 10', 'CL', '124', '2026-27', 11);
  end if;
